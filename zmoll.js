        ...
        geterateCompareTRs : function (iCol, sDataType) { //比较函数，用于sort排序用  
            return function compareTRs(oTR1, oTR2) {
                ...
                if (oTR1.cells[iCol].getAttribute("value")) { //用于高级排序，比如图片，添加一个额外的属性来排序  
                    vValue1 = ZK.convert(oTR1.cells[iCol].getAttribute("value"), sDataType);
                    vValue2 = ZK.convert(oTR2.cells[iCol].getAttribute("value"), sDataType);
                } else {
                    vValue1 = ZK.convert(oTR1.cells[iCol].firstChild.nodeValue, sDataType);
                    vValue2 = ZK.convert(oTR2.cells[iCol].firstChild.nodeValue, sDataType);
                }
                if (vValue1 < vValue2) {
                    return -1;
